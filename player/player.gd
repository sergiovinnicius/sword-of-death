...
func update_attack_cooldown(delta: float) -> void:
					...
					if is_attacking:
						...
						if attack_cooldown <= 0.0:
							...
							is_running = false 
							animation_player.play("idle")
							# OBS: Código ficou um tempo sem funcionar devido apenas as linhas de comando do if não estarem alinhadas!!!
...
func update_hitbox_detection(delta: float) -> void:
	...
	#Frequenica de Dano (2x por segundo)
	hitbox_cooldown = 0.5
	#HitBoxArea
